...
window.onload = async function (): Promise<void> {
    ...
    if (action === StreamClientScrcpy.ACTION && typeof parsedQuery.get('udid') === 'string') {
        client = StreamClientScrcpy.start(parsedQuery);

        // Attach button listeners after client is created
        document.getElementById('recordBtn')?.addEventListener('click', () => client?.startRecording());
        document.getElementById('stopRecordBtn')?.addEventListener('click', () => client?.stopRecording());
        document.getElementById('saveBtn')?.addEventListener('click', () => client?.saveRecordedActions());
        ...
        document.getElementById('loadInput')?.addEventListener('change', (e: Event) => {
            ...
            if (input.files && input.files[0]) {
                client?.loadRecordedActions(input.files[0]);
            }
        });
        document.getElementById('replayBtn')?.addEventListener('click', () => client?.replayRecordedActions());
        return;
    }
